...
app.post("/webhook-whatsapp", async (req, res) => {
  try {
    ...
    console.log("Mensaje recibido:", entry);
    const text = entry;
...
function fueraDeTolerancia(horaActual, horaBase, tolerancia) {
  ...
  const [h1, m1] = horaActual.replace("a.m.", "").replace("p.m.", "").split(':').map(Number);
  const [h2, m2] = horaBase.replace("a.m.", "").replace("p.m.", "").split(':').map(Number);
